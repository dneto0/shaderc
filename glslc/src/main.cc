...
Options:
  ...
  -fuav-binding-base [stage] <value>
                    ...
                    number for unordered access views (UAV).  Optionally only
                    set it for a single shader stage.  Only affects HLSL.
  -fentry-point=<name>
...
bool GetOptionalStageThenOffsetArgument(const shaderc_util::string_piece option,
  ...
  if (stage != shaderc_glsl_infer_from_source) {
    ++argi;
    if (argi + 1 >= argc) {
      *errs << "glsc: error: Option " << option << " with stage "
  ...
  if (!num_parse.first) {
    *errs << "glsc: error: invalid offset value " << argv[argi + 1] << " for "
          << option << std::endl;
    return false;
  }
  ++argi;
